...
pub fn encrypt(plain_text: &str, key: &[u8]) -> Result<String> {
    ...
    let ciphertext = cipher.encrypt(nonce, plain_text.as_bytes()).map_err(|e| anyhow!(e.to_string()))?;
...
pub fn decrypt(crypto_text: &str, key: &[u8]) -> Result<String> {
    ...
    let decrypted = cipher.decrypt(nonce, ciphertext).map_err(|e| anyhow!(e.to_string()))?;
