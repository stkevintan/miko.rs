<script lang="ts">
    ...
    async function saveUser() {
        ...
        try {
            const endpoint =
                showDialog === 'create' ? '/createUser' : '/updateUser';
            ...
            toast.success(
                `User ${showDialog === 'create' ? 'created' : 'updated'} successfully`,
            );
            showDialog = null;
            await fetchUsers();
        } catch (error: any) {
...
{#if showDialog}
    <div
        class="mb-6 bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 shadow-sm p-6"
    >
        <h3 class="text-lg font-semibold mb-4 text-gray-900 dark:text-white">
            {showDialog === 'create'
                ? 'Create New User'
                : `Edit User: ${dialogUser.username}`}
        </h3>
