<script lang="ts">
    ...
    async function fetchMusicFolders() {
        ...
        try {
            const response =
                await api.get<SubsonicResponse>('/getMusicFolders');
            ...
            if (musicFolders.length > 0 && !currentDirectory) {
                ...
                selectedFolderId =
                    firstFolder.directoryId || firstFolder.id.toString();
...
{#snippet row(item: Song)}
    <td class="px-4 py-3">
        {#if item.isDir}
            ...
            >
                <div
                    class="w-10 h-10 rounded-lg bg-gray-100 dark:bg-gray-800 flex items-center justify-center text-gray-500 group-hover/link:bg-orange-100 dark:group-hover/link:bg-orange-900/30 group-hover/link:text-orange-600 transition-colors"
                >
                    <Folder size={20} />
                </div>
                <div
                    class="text-sm font-semibold text-gray-900 dark:text-white group-hover/link:text-orange-600 truncate transition-colors"
                >
                    {item.title}
                </div>
            </button>
    ...
    <td class="px-4 py-3">
        <div
            class="text-[10px] font-mono text-gray-400 dark:text-gray-500 truncate max-w-[150px] sm:max-w-xs"
            title={item.path}
        >
...
<div
    class="flex-1 flex flex-col min-h-0 bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 shadow-sm overflow-hidden"
>
    <div
        class="p-4 border-b border-gray-100 dark:border-gray-800 flex items-center gap-4"
    >
        {#if history.length > 0}
            <button
                onclick={goBack}
            ...
            <div class="flex items-center gap-2">
                ...
                >
                    {#each musicFolders as folder}
                        <option
                            value={folder.directoryId || folder.id.toString()}
