<script lang="ts">
    ...
    async function fetchScanStatus() {
        try {
            const res = await api.get('/getScanStatus');
...
<div class="relative">
    <Dropdown triggerMode="hover" align="right">
        {#snippet trigger()}
            ...
            >
                {#if scanStatus?.scanning}
                    ...
                    <span
                        class="ml-2 text-xs font-medium text-orange-600 dark:text-orange-400"
                    >
        ...
        {#snippet content()}
            <div
                class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-xl overflow-hidden py-1 w-48"
            >
                <div
                    class="px-4 py-2 text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wider border-b border-gray-100 dark:border-gray-700 mb-1"
                >
                    Library Scan
                </div>
                <button
